'''
Nov 1 Test SIX, Jimmy
**Updates:
1) Simplified so  python only prints the one number it sent to Arduino
...
def on_wekinator_output(address, *args):
    ...
    if a_label is not None and b_label is not None:
        ...
        if key in WEKINATOR_TO_ARDUINO_MAP:
            ...
            print(f"[INFO] Received: {arduino_cmd}")
    ...
    if ser is not None:
        try:
            ...
            print(f"[INFO]Sent to Arduino: {line.strip()}")
        except Exception as e:
